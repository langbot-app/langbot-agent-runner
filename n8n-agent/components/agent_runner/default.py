...
class DefaultAgentRunner(AgentRunner):
    """Default AgentRunner for n8n Workflow Agent.
    ...
    async def run(self, ctx: AgentRunContext) -> AgentRunResult:
        ...
        Stub implementation: echoes webhook URL.
        """
        webhook_url = ctx.config.get("webhook-url", "http://localhost")
        text = ctx.input.to_text()
        message = Message(
            role="assistant",
            content=f"[stub] n8n webhook {webhook_url}: {text}",
        )
